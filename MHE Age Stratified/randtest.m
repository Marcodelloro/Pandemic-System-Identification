...
ymeas.old = [ S_data.old';  [I_data.old(1) I_data.old']./Npop; D_data.old'./Npop; (T1_data.old + T2_data.old)'./Npop; H_data.old'./Npop; E_data.old'./Npop ];  
ymeas.ger = [ S_data.ger';  [I_data.ger(1) I_data.ger']./Npop; D_data.ger'./Npop; (T1_data.ger + T2_data.ger)'./Npop; H_data.ger'./Npop; E_data.ger'./Npop ]; 


std_u40 = std(ymeas.u40,1,2);
std_mid = std(ymeas.mid,1,2);
std_old = std(ymeas.old,1,2);
std_ger = std(ymeas.ger,1,2);

a = ymeas.u40./max(ymeas.u40, [], 2);
b = ymeas.mid./max(ymeas.mid, [], 2);
c = ymeas.old./max(ymeas.old, [], 2);
d = ymeas.ger./max(ymeas.ger, [], 2);

figure(1)
plot(a(1,:), 'DisplayName', 'u40');  % Plot 'a' with a legend label
hold on;  % Hold on to add more plots to the same figure
plot(a(2,:), 'DisplayName', 'mid');  % Plot 'b' with a legend label
plot(a(3,:), 'DisplayName', 'old ');  % Plot 'c' with a legend label
plot(a(4,:), 'DisplayName', 'ger');  % Plot 'd' with a legend label
title('Normalized Measurements - S');  % Title of the plot
legend('show');  % Display the legend
grid on;  % Turn on the grid

figure(2)
plot(a(6,:), 'DisplayName', 'u40');  % Plot 'a' with a legend label
hold on;  % Hold on to add more plots to the same figure
plot(b(6,:), 'DisplayName', 'mid');  % Plot 'b' with a legend label
plot(c(6,:), 'DisplayName', 'old ');  % Plot 'c' with a legend label
plot(d(6,:), 'DisplayName', 'ger');  % Plot 'd' with a legend label
title('Normalized Measurements - S');  % Title of the plot
legend('show');  % Display the legend
grid on;  % Turn on the grid
