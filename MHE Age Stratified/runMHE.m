function [res, dltp] = runMHE(N_mhe, N_sim, Ts, ymeas_struct, c_struct)

    % The following FUNCTION 'residuals' performs a MHE optimization on a reduced interval
    % of 140 days (20 weeks) in order to evaluate the residuals between simulation and data.
    % The active parameters which influence the results of the algorithm are the weights of the 
    % cost fucntion, whioch are imposed as random variables changing every time the function is called. 


   % ------------------------------------------------------------------------------------------------------- %

    % RunMHE - Function to perform Moving Horizon Estimation over N weeks.
    
    % INPUTS:
    %   N_mhe              - Estimation horizon (e.g., 21 for 3 weeks)
    %   N_sim              - Days over whcih the MHE algorithm is optimsed
    ...
    %   params             - Model parameters (lambdas, bounds, etc.)

    % OUTPUT: 
    %   residuals          - Matrix of the residuals between MHE simulations and actual measured data

   if N_sim < 140 || N_sim > 399
